...
export const CompleteWorkoutScreen = ({ route, navigation }: any) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isLocal) {
        // Persist completed session to local history
        await workoutHistory.save({
          ...
          totalSets,
          completedSets,
          estimatedTime: workout.estimatedTime || 0,
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalButtons}>
              <TouchableOpacity style={styles.modalCancelBtn} onPress={() => { setShowTemplateModal(false); navigation.popToTop(); }}>
...
const styles = StyleSheet.create({
  ...
  textInput: {
    backgroundColor: '#0a1628', borderRadius: 12, padding: 14, fontSize: 15, color: '#fff',
    minHeight: 100, borderWidth: 1, borderColor: '#1a3a6b',
  },
  quoteCard: {
    flexDirection: 'row', backgroundColor: '#0d1f3c', borderRadius: 16,
    padding: 18, marginBottom: 20, borderLeftWidth: 3, borderLeftColor: '#4A9EFF',
  },
  ...
  completeBtn:  { backgroundColor: '#26de81', borderRadius: 14, padding: 16, alignItems: 'center', elevation: 4 },
  ...
  modalInput: {
    backgroundColor: '#0a1628', borderRadius: 12, padding: 14, fontSize: 16, color: '#fff',
